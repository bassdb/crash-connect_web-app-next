...
export default function TypeDialog({ mode, action, initialValues, children }: TypeDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='sm:max-w-lg'>
        <DialogHeader>
          <DialogTitle>{mode === 'create' ? 'Neuen Typ erstellen' : 'Typ bearbeiten'}</DialogTitle>
        </DialogHeader>
        <form action={handleSubmit} className='space-y-4'>
          ...
          <div className='grid gap-2'>
            ...
            <Input id='name' name='name' required defaultValue={initialValues?.name ?? ''} placeholder='z. B. Fußball' />
          </div>
