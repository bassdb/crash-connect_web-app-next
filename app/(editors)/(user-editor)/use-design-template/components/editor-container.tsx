...
export default function EditorContainer({ templateId, designTemplateData }: EditorContainerProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === 'Space') {
        setCanPan(true)
        console.log('canPan true triggered by space')
        document.body.style.cursor = 'grab'
      }
      if (e.code === 'Escape') {
        setCanPan(false)
        ...
        console.log('canPan to false triggered by escape')
        document.body.style.cursor = 'default'
      }
      if (e.code === 'Equal' || e.code === 'Plus') {
        setScale(Math.min(scale + 0.1, 4))
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      if (e.code === 'Space') {
        setCanPan(false)
        console.log('canPan to false triggered by space')
      }
    }
  ...
  }, [setCanPan, scale, setScale, fabricCanvas, setSelectedObject])
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    if (canPan) {
      setStartPan(false)
      console.log('canPan to false triggered by mouse up')
    }
  }
